export function USPs() {
    return (
        <section className="bg-gray-900 text-white">
            <div className="mx-auto max-w-screen-xl px-4 py-8 sm:px-6 sm:py-12 lg:px-8 lg:py-16">
                <div className="mx-auto max-w-3xl text-center">
                    <h2 className="text-3xl font-bold sm:text-4xl">Waarom kiezen voor ons aannemersbedrijf?</h2>

                    <p className="mt-4 text-gray-300">
                        Bij ons bedrijf combineren we vakmanschap met innovatie om elke bouw- en renovatieklus naar een
                        hoger niveau te tillen. Ontdek waarom zoveel klanten ons vertrouwen voor hun bouwprojecten. Hier
                        zijn drie redenen waarom we uitblinken in wat we doen:
                    </p>
                </div>

                <div className="mt-8 grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3">
                    <div className="block rounded-xl border border-gray-800 p-8 shadow-xl transition hover:border-pink-500/10 hover:shadow-primary/10">
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
                            fill="#33d037"
                            width="48px"
                            height="48px"
                            version="1.1"
                            id="Layer_1"
                            viewBox="0 0 512 512"
                            xmlSpace="preserve"
                        >
                            <g>
                                <g>
                                    <g>
                                        <path d="M490.667,74.667H167.979c-9.472-36.807-42.88-64-82.645-64C38.202,10.667,0,48.869,0,96v320     c0,0.461,0.012,0.914,0.034,1.36c0.73,46.501,38.624,83.973,85.299,83.973h405.333c11.782,0,21.333-9.551,21.333-21.333V96     C512,84.218,502.449,74.667,490.667,74.667z M42.667,96c0-23.567,19.099-42.667,42.667-42.667S128,72.433,128,96v246.081     c-12.551-7.26-27.123-11.415-42.667-11.415c-15.543,0-30.116,4.155-42.667,11.415V96z M469.333,458.667h-384     c-23.567,0-42.667-19.099-42.667-42.667s19.099-42.667,42.667-42.667S128,392.433,128,416c0,14.222,10.667,21.333,21.333,21.333     h0l0,0c10.667,0,21.333-7.111,21.333-21.333V117.333h298.667V458.667z" />
                                        <path d="M234.667,352h170.667c11.782,0,21.333-9.551,21.333-21.333V224c0-6.715-3.161-13.038-8.533-17.067l-85.333-64     c-7.585-5.689-18.015-5.689-25.6,0l-85.333,64c-5.372,4.029-8.533,10.352-8.533,17.067v106.667     C213.333,342.449,222.885,352,234.667,352z M256,234.667l64-48l64,48v74.667H256V234.667z" />
                                        <path d="M403.836,373.333h-41.173c-11.782,0-21.333,9.551-21.333,21.333c0,11.782,9.551,21.333,21.333,21.333h41.173     c11.782,0,21.333-9.551,21.333-21.333C425.169,382.885,415.618,373.333,403.836,373.333z" />
                                    </g>
                                </g>
                            </g>
                        </svg>

                        <h2 className="mt-4 text-xl font-bold text-white">Op Maat Gemaakte Oplossingen</h2>

                        <p className="mt-1 text-sm text-gray-300">
                            Elk project is uniek, en daarom bieden we op maat gemaakte bouwoplossingen die perfect
                            aansluiten bij uw specifieke wensen en eisen. Van de eerste schets tot het laatste detail,
                            uw visie staat bij ons centraal.
                        </p>
                    </div>

                    <div className="block rounded-xl border border-gray-800 p-8 shadow-xl transition hover:border-pink-500/10 hover:shadow-primary/10">
                        <svg
                            fill="#33d037"
                            width="48px"
                            height="48px"
                            viewBox="0 0 16 16"
                            xmlns="http://www.w3.org/2000/svg"
                        >
                            <path d="M8 .5A7.76 7.76 0 0 0 0 8a7.76 7.76 0 0 0 8 7.5A7.76 7.76 0 0 0 16 8 7.76 7.76 0 0 0 8 .5zm6.71 6.8L13.48 7c-.25-.07-.27-.09-.29-.12-.15-.2-.32-.47-.48-.73 0-.09-.13-.23-.16-.31s.35-.6.51-.84a2.43 2.43 0 0 1 .59-.45 5.87 5.87 0 0 1 1.06 2.75zM8 1.75l-.09.17a.19.19 0 0 1 0-.1c0 .06-.15.15-.25.25l-.3.29a.85.85 0 0 0-.08 1.08h-.12a1.05 1.05 0 0 0-.81.42 1.27 1.27 0 0 0-.2 1.07V5a3 3 0 0 0-.43.11l-.24.08-.64.21a1.2 1.2 0 0 0-.81.8 1 1 0 0 0 .2.93 5.67 5.67 0 0 0 1.38 1.09 4.17 4.17 0 0 0 1.67.65h1.68a1.2 1.2 0 0 1 1.04.51.49.49 0 0 1 .13.43.77.77 0 0 1-.15.35 2.71 2.71 0 0 0-.95 1.61 11.11 11.11 0 0 1-.48 1.38c-.12.31-.23.61-.31.85a3.32 3.32 0 0 1-1-.08 3.28 3.28 0 0 0-.5-2.12 2.24 2.24 0 0 1-.53-1.42 2.11 2.11 0 0 0-1.47-2.29 10.81 10.81 0 0 1-2.9-2.64A6.79 6.79 0 0 1 8 1.75zM1.25 8a5.64 5.64 0 0 1 .12-1.16 10.29 10.29 0 0 0 2.94 2.42c.6.22.69.45.69 1.12a3.45 3.45 0 0 0 .86 2.27A3.05 3.05 0 0 1 6 14a6.35 6.35 0 0 1-4.75-6zm8.32 6.08c0-.15.12-.32.18-.48a10.2 10.2 0 0 0 .55-1.6 1.55 1.55 0 0 1 .54-.86 1.91 1.91 0 0 0 .57-1.3 1.71 1.71 0 0 0-.47-1.27 2.45 2.45 0 0 0-2-.9H7.35a4.77 4.77 0 0 1-2-1.11l.47-.16.27-.08a.79.79 0 0 1 .38-.07l.09.15a.64.64 0 0 0 .81.29.65.65 0 0 0 .34-.8v-.18c-.11-.3-.24-.72-.32-1A1.42 1.42 0 0 0 8.68 4a1 1 0 0 0-.18-1 3.44 3.44 0 0 0 .33-.34 1 1 0 0 0 .22-.8 6.93 6.93 0 0 1 3.73 1.8 3 3 0 0 0-.79.7 9.14 9.14 0 0 0-.64 1.09 1.46 1.46 0 0 0 .24 1.39c.18.31.38.61.56.86a1.58 1.58 0 0 0 1 .58c.22.06 1 .22 1.55.33a6.44 6.44 0 0 1-5.13 5.47z" />
                        </svg>

                        <h2 className="mt-4 text-xl font-bold text-white">Duurzaam en Milieuvriendelijk</h2>

                        <p className="mt-1 text-sm text-gray-300">
                            We zetten ons in voor duurzaam bouwen. Door gebruik te maken van milieuvriendelijke
                            materialen en methodes dragen we niet alleen zorg voor uw ruimte, maar ook voor de planeet.
                        </p>
                    </div>

                    <div className="block rounded-xl border border-gray-800 p-8 shadow-xl transition hover:border-pink-500/10 hover:shadow-primary/10">
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
                            width="48px"
                            height="48px"
                            viewBox="0 0 24 24"
                            fill="none"
                        >
                            <path
                                d="M5.06152 12C5.55362 8.05369 8.92001 5 12.9996 5C17.4179 5 20.9996 8.58172 20.9996 13C20.9996 17.4183 17.4179 21 12.9996 21H8M13 13V9M11 3H15M3 15H8M5 18H10"
                                stroke="#33d037"
                                strokeWidth="2"
                                strokeLinecap="round"
                                strokeLinejoin="round"
                            />
                        </svg>

                        <h2 className="mt-4 text-xl font-bold text-white">Tijdige Oplevering</h2>

                        <p className="mt-1 text-sm text-gray-300">
                            We begrijpen het belang van tijd in bouwprojecten. Ons team zorgt ervoor dat elk project
                            strikt volgens schema verloopt, zodat u op tijd kunt genieten van uw vernieuwde ruimte.
                        </p>
                    </div>
                </div>
            </div>
        </section>
    );
}
